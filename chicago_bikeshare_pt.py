...
#
# Nota:
# Implementada com um 'for loop e list.append()' o corpo da função ficaria assim:
#    column_list = []
#    for sample in data:
...
sorted_durations = sorted([float(i) for i in trip_duration_list])
min_trip = sorted_durations[0]
max_trip = sorted_durations[-1]
mean_trip = sum(sorted_durations)/len(sorted_durations)
if len(stdl) % 2 == 0:
    median_trip = (sorted_durations[len(sorted_durations)//2] + sorted_durations[len(sorted_durations)//2 + 1]) / 2
else: 
    median_trip = sorted_durations[len(sorted_durations)//2]
...
print("Você vai encarar o desafio? Sim!")
answer = "yes"
